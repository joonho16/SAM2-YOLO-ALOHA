...
class FusionModel(nn.Module):
    def __init__(self, ratio=0.6):
        ...
        self.block1 = Block([c1, c1, 'M'], in_channels=3, L=3, first_block=True, D_in_channels=False)
        self.block2 = Block([c2, c2, 'M'], in_channels=c1, L=2)
        self.block3 = Block([c3, c3, c3, c3, 'M'], in_channels=c2, L=1)
        self.block4 = Block([c4, c4, c4, c4, 'M'], in_channels=c3, L=0)
        self.block5 = Block([c4, c4, c4, c4], in_channels=c4, L=0)
    ...
    def forward(self, RGBT):
        ...
        x = self.reg_layer(x)
        # plt.imshow(torch.abs(x[0][0].cpu()))
        # plt.show()
        return torch.abs(x)
...
class Block(nn.Module):
    ...
    def fuse(self, RGB, T):
        ...
        rgb_query = self.RGB_query(RGB_m).reshape(batch_size, adapt_channels, -1).permute(0, 2, 1)
        rgb_key = self.RGB_key(RGB_m).reshape(batch_size, adapt_channels, -1)
        rgb_value = self.RGB_value(RGB_m).reshape(batch_size, adapt_channels, -1).permute(0, 2, 1)

        batch_size = T_m.size(0)
        T_query = self.T_query(T_m).reshape(batch_size, adapt_channels, -1).permute(0, 2, 1)
        T_key = self.T_key(T_m).reshape(batch_size, adapt_channels, -1)
        T_value = self.T_value(T_m).reshape(batch_size, adapt_channels, -1).permute(0, 2, 1)
